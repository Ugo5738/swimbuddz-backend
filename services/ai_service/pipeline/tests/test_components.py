...
def test_pose_recovery_replaces_near_recovery_instances():
    ...
    ctx.instances = [
        Instance(Phase.RECOVERY, 0, 0.2, 0.5, 0.35, arm="near"),  # VLM near → replaced
    ...
    assert any(
        i.phase == Phase.RECOVERY and i.arm == "far" for i in ctx.instances
    )  # kept
    assert any(i.phase == Phase.GLIDE for i in ctx.instances)  # kept
    assert ctx.pose_recovery["count"] == 3
    assert cache["pose_recovery"]["peaks_s"] == [0.8, 2.1, 3.4]  # cached for $0 replay
    # cache["instances"] is what the API surfaces as result.instances (the drilldown
    # source) — it must reflect the pose splice, not the VLM grouping.
    cached_near = [
        i for i in cache["instances"] if i["phase"] == "recovery" and i["arm"] == "near"
    ]
    assert [round(i["peak_s"], 1) for i in cached_near] == [0.8, 2.1, 3.4]

    # replay: count_fn must NOT run; the same instances re-splice from the cache
    async def boom(ctx):
...
def test_collate_counts_pose_segmented_instances():
    ...
    ctx.instances = [
        ...
        for i, t in enumerate((0.8, 2.1, 3.4))
    ]
    ctx.pose_recovery = {"count": 3, "refused": False, "detection_rate": 0.96}
    res = asyncio.run(CollateComponent().run(ctx))
    f = res.findings[0]
    assert f.extra["recovery_count_hedged"] == 3  # counts the pose instances
    assert f.extra["count_source"] == "pose"
    assert f.confidence == 0.8
    assert "~3" in f.observation
