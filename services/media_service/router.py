...
async def list_albums(
    album_type: Optional[str] = None, db: AsyncSession = Depends(get_async_db)
):
...
async def upload_photo(
    ...
    file_url, thumbnail_url = await storage_service.upload_photo(
        file_data, file.filename or "photo.jpg", file.content_type
    )
...
async def get_featured_photos(limit: int = 6, db: AsyncSession = Depends(get_async_db)):
...
async def tag_member_in_photo(
    ...
    existing_query = select(PhotoTag).where(
        PhotoTag.photo_id == photo_id, PhotoTag.member_id == member_id
    )
    ...
    # Create tag
    db_tag = PhotoTag(photo_id=photo_id, member_id=member_id)
    db.add(db_tag)
    await db.commit()
