...
async def seed_academy_data():
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            res = await client.post(login_url, json=login_payload, headers=login_headers)
        ...
        try:
            res = await client.post(f"{base_url}/programs", json=program_payload, headers=headers)
            if res.status_code == 200:
                data = res.json()
                program_id = data["id"]
        ...
        try:
            res = await client.post(f"{base_url}/cohorts", json=cohort_payload, headers=headers)
            if res.status_code == 200:
                data = res.json()
