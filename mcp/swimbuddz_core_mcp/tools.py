...
async def sign_in_to_session(
    ...
    return await _make_request(
        "POST", f"/api/v1/attendance/sessions/{session_id}/sign-in", token, json=payload
    )
...
async def create_announcement(
    token: str, title: str, body: str, category: str, is_pinned: bool = False
) -> Dict[str, Any]:
    ...
    return await _make_request(
        "POST", "/api/v1/communications/announcements/", token, json=payload
    )
